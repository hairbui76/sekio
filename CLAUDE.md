...
- **Never resize a bitmap with `image`'s own `resize`.** It costs more than decoding the file (320 ms on a 24 MP JPEG) and its cheapest usable filter is bilinear, which is what made previews look soft. Every renderer goes through `render/resample.rs`, which is `fast_image_resize` with Lanczos3 — sharper *and* 2–4x faster, and pure Rust (`std::arch` SIMD), so the no-C-toolchain rule holds. Same rule as `max_lines`: a frontend passes the `image_max_dim` it can actually paint, per request, rather than a fixed number.
- **Pixels are not points, and egui will not tell you.** A texture is sized in screen pixels; `ui.available_size()` and everything passed to `ui.image` are in points. Painting a texture at "its own size" magnifies it by `pixels_per_point`, which silently blurred every GUI picture on any display above 100% — invisible on this machine, universal on Windows. Divide by `ui.pixels_per_point()` before laying a texture out (`app.rs::fit`), and measure surfaces in pixels before asking core for an image (`app.rs::image_pixels`).
